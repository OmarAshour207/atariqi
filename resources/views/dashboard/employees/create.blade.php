            ...
            <form method="POST" action="{{ route('employees.store') }}">
                ...
                @foreach($permissions as $permission)
                    <div class="form-check">
                        <input type="checkbox" name="permission_ids[]" value="{{ $permission->id }}" id="perm{{ $permission->id }}" class="form-check-input">
                        <label for="perm{{ $permission->id }}" class="form-check-label">{{ __($permission->name) }}</label>
                    </div>
                @endforeach
                <hr>
                ...
                @foreach($pages as $page)
                    <div class="form-check">
                        <input type="checkbox" name="page_ids[]" value="{{ $page->id }}" id="page{{ $page->id }}" class="form-check-input">
